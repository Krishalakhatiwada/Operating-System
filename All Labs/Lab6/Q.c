...
int main() {
    ...
    while (1) {
        printf("\nProcess scheduling algorithm: \n\
                ...
                3. Shortest Remaining Time Next(preemptive) \n\
                4. Round Robin \n\
                5. Exit \n");
...
void FCFS(int n, int *TATime, int *arrivalTime, int *burstTime, int *waitingTime, int *p) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        *(waitingTime + i) = *(TATime + i) - *(burstTime + i);
        printf("\nP[%d]\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d",*(p+i),*(arrivalTime+i),*(burstTime + i), *(completionTime));
    }
...
void SJF(int n, int *TATime, int *arrivalTime, int *burstTime, int *waitingTime, int *p) {
    ...
    for (i = 1; i < n; i++) {
        ...
        /* when you get less burst time process, swap p, arrivalTime, burstTime arrivalTime position 2,
        and when getting 2nd less burst time swap arrivalTime position 3rd and so on. */
        swap(&p[i], &p[pos]);
    ...
    for (i = 0; i < n; ++i) {
        *(TATime + i) = *(completionTime + i) - *(arrivalTime + i);
        *(waitingTime + i) = *(TATime + i) - *(burstTime + i);
        printf("\nP[%d]\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d",*(p+i),*(arrivalTime+i),*(burstTime + i),
        completionTime);
    }
...
void SRTN(int n) {
    ...
    for (i = 0; i < n; i++) {
        printf("\nP[%d]\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d",i + 1, ari[i],waiting[i],finish[i] + 1,
        finish[i] + 1);
...
// if process is completed
    ...
    for (i = 0; i < n; i++) {
        printf("\nP[%d]\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d", a[i].pid, a[i].AT, a[i].BT, a[i].TAT, a[i].WT);
    }
    printf("\n\nThe average waiting time is %.3f\n", Avg_WT);
    printf("The average TurnAround time is %.3f\n", Avg_TAT);
}
